...
export class GLHelper {
  ...
  private isRotateCamera: boolean[];
  ...
  constructor(gl: WebGL2RenderingContext, shaderProgram: WebGLProgram) {
    ...
    this.isRotateCamera = [false, false, false];
  ...
  getUniformLocation() {
    return {
      ...
      uModelViewM: this.gl.getUniformLocation(
        this.shaderProgram,
        "uModelViewM"
      ),
      ...
      uWorldCameraPos: this.gl.getUniformLocation(
        this.shaderProgram,
        "uWorldCameraPos"
      ),
  ...
  drawScene(): // buffVertex: BufferVertex,
  // diffTime: number,
  ...
  void {
    ...
    // // rotate camera
    for (let i = 0; i < 3; i++) {
      if (this.isRotateCamera[i]) {
        this.rotationModel[i] += 0.01;
      }
    }
    ...
    mModel = m4.zRotate(
      m4.yRotate(
        m4.xRotate(mModel, this.rotationModel[0]),
        this.rotationModel[1]
      ),
      this.rotationModel[2]
    );
    mModel = m4.translate(
      mModel,
      -(minX + maxX) / 2,
  ...
  initNodes(id: number): void {
    ...
    if (this.isAnimate) {
      ...
      degree = this.nodeListData[id].start_degree + this.theta[id];
      if (degree > this.nodeListData[id].max_degree) {
        degree = this.nodeListData[id].max_degree;
      }
      if (degree < this.nodeListData[id].min_degree) {
        degree = this.nodeListData[id].min_degree;
      }
    }
    ...
    switch (rotAxis) {
      case 0:
        // mNode = m4.xRotate(mNode, this.nodeListData[id].start_degree);
        mNode = m4.xRotate(mNode, degree);
        break;
      case 1:
        // mNode = m4.yRotate(mNode, this.nodeListData[id].start_degree);
        mNode = m4.yRotate(mNode, degree);
        break;
      case 2:
        // mNode = m4.zRotate(mNode, this.nodeListData[id].start_degree);
        mNode = m4.zRotate(mNode, degree);
        break;
      default:
        break;
    }
  ...
  setRotateCamera(axis) {
    switch (axis) {
      case 0:
        this.isRotateCamera[0] = true;
        this.isRotateCamera[1] = false;
        this.isRotateCamera[2] = false;
        break;
      case 1:
        this.isRotateCamera[0] = false;
        this.isRotateCamera[1] = true;
        this.isRotateCamera[2] = false;
        break;
      case 2:
        this.isRotateCamera[0] = false;
        this.isRotateCamera[1] = false;
        this.isRotateCamera[2] = true;
        break;
      default:
        this.isRotateCamera[0] = false;
        this.isRotateCamera[1] = false;
  ...
  setCenterVal(val, axis) {
    this.center[axis] = val;
  }
  setUpVal(val, axis) {
    this.up[axis] = val;
  }

  setRusukAngle(id: number, angle) {
    // this.nodeListData[id].start_degree =
    // let axis = this.nodeListData[id].rotation_axis;
    this.theta[id] += angle;
  }

  setAllPartModel(isAnimate) {
